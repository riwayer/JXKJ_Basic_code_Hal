...
/***************************************************************************************
  *本文件主要来自B站江协科技对0.96寸OLED的移植
  *原教程：https://www.bilibili.com/video/BV1EN41177Pc?t=555.7
...
/**
  ...
  * 返 回 值：无
  */
void OLED_WriteData(const uint8_t *Data, uint16_t Count)//写数据
{
    uint8_t buf[256];
    buf[0] = 0x40;
...
{
	HAL_StatusTypeDef Status = HAL_I2C_IsDeviceReady(&hi2c1, 0x78, 5, HAL_MAX_DELAY);
	if (Status == HAL_OK) {
		return 1;
	}
	else {
...
void OLED_Init(void)
{
	...
	OLED_Clear();				//清空显存数组
	OLED_Updata();				//更新显示，清屏，防止初始化后未显示内容时花屏
}
